...
同じ内容をスライドにした資料（WG 立ち上げガイド 00〜03）が
<https://funitclub.org/guide/> にある。画面の写真つきで追いたいときはそちらを。
食い違ったときは、この文書を正とする。
...
| 困ること | Docker（作業部屋）がしてくれること |
|---|---|
| 「自分の PC では動くのに」が起きる | 全員が同じ中身の作業部屋で動かすので、差が出ない |
| Windows と Mac で手順やコマンドが違う | 作業部屋の中はどちらも Linux。同じコマンドで済む |
| 本番と手元で Python の版がずれる | 本番と同じ Python 3.11 にしてある |
| 環境を壊すと戻せない | 作り直せば元どおり。PC 本体には何も残らない |
| Python を入れる手順が難しい | Docker Desktop を起動するだけ。中身は触らない |

コードを書くのは VS Code、動かすのは作業部屋の中。Docker Desktop は起動しておくだけで、
中を直接さわることはない。
...
- パスワードや API キーは **コードに書かない・コミットしない**。一度 push すると、消しても
  履歴に残る。homepage は公開リポジトリなので誰でも見られ、WG のリポジトリ（非公開）も
  WG の全員が見られる。
