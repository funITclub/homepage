...
どちらも 2026-09-19 に検討して保留にしたもの。始めるときはここから。

**1. Chat スペースへの自動登録（大学の了承は得ている）**
いまは WG の Chat スペースのリンクを Classroom の授業に置き、本人が入る（スペースは大学内に
公開が前提）。スペースを「メンバーのみ」にしたいときはこれを入れる。

- 確認のメールのリンクを開いた時点で、運営のアカウントの権限で本人をスペースに追加する
  （Chat API の `spaces.members.create`、メンバーは `users/<アドレス>`）。アドレスを入れた時点では
  追加しない（他人のアドレスで勝手に入れられるため）
- そのため確認のリンクにアドレスを**暗号化して**入れる（DB・ログには残さない方針は変えない）
- 準備：Google Cloud で Google Chat API を有効にし、Chat API の「構成」でアプリ名などを保存する。
  運営の許可を `chat.memberships` を足して取り直す（Key Vault のトークンも入れ替え）。
  運営のアカウントを各 WG のスペースのメンバー（できれば管理者）にしておく
- サイト：WG紹介に「Chat スペースの URL」を足す。完了画面の Chat の案内を「追加しました」に変える

**2. WG ごとに Classroom でも分ける**
いまは WG をクラブのクラスの「授業」のトピックにしていて、クラスの全員に見える。
WG のメンバーだけに見せたいときの案。

- (a) WG ごとに別のクラスを作り、「WG に参加」のときにクラスコードで本人をそのクラスに登録する
  （本人の権限で `courses.students.create` に `enrollmentCode` を付ける方法。一度作りかけて外した）
- (b) クラブのクラスのまま、投稿ごとに対象の生徒を選ぶ。人が入るたびに直す必要があり、自動化は難しい

### そのほか

- WG の成果物は Web ページ（動いているアプリ）だけで、ソースコードは成果物に含めない。
  WG のリポジトリは完成後も非公開のまま。WG のアプリを動かす仕組み（Container Apps と
  ghcr.io の構成案）を作るときは、非公開のリポジトリから作った**非公開のイメージ**を
  Azure 側で取り出せるようにする（イメージを取るための認証が要る）。
